...
PROJECT_DIR = Path(__file__).parent.parent.parent  # Go up to .claude/skills/rent
...
# Cache location: .claude/skills/rent/cache/
...
def save_cache(scored_bounties):
    """Save scored bounties to cache with timestamp and generate TXT file."""
    ...
    # Generate human-readable TXT file
    generate_cache_txt(cache)


def generate_cache_txt(cache):
    """Generate human-readable TXT file from cache JSON."""
    bounties = cache.get("bounties", [])
    last_call = cache.get("last_call", "unknown")

    lines = [
        "═" * 80,
        "BOUNTY HUNTER — RANKED OPPORTUNITIES",
        "═" * 80,
        f"",
        f"Scored via: Grok 4.1 Fast Reasoning",
        f"Last updated: {last_call[:19]}",
    ...
    lines.extend([
        "",
        "─" * 80,
        "View full details at: https://rentahuman.ai/bounties/[ID]",
        "═" * 80,
    ])
...
def scan(hours=1000, limit=20, force=False):
    ...
    if force:
        ...
        if not candidates:
            return f"No bounties found ({len(bounties)} total checked)."
