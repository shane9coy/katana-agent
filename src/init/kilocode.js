...
async function init(opts) {
  const targetDir = path.join(process.cwd(), '.kilocode');
  const templateDir = path.join(__dirname, '../../templates/kilocode');

  console.log('');
  console.log(chalk.bold('⚡ Katana Agent → KiloCode'));
  console.log('');

  let action = 'fresh';

  if (fs.existsSync(targetDir)) {
    action = await checkConflict('.kilocode');
    if (action === 'cancel') { console.log(chalk.dim('  Cancelled.')); return; }
    if (action === 'backup') {
      const backupName = `.kilocode.backup-${Date.now()}`;
      fs.renameSync(targetDir, path.join(process.cwd(), backupName));
      console.log(chalk.green(`  ✓ Backed up → ${backupName}`));
    }
    if (action === 'replace') {
      fs.rmSync(targetDir, { recursive: true, force: true });
      console.log(chalk.green('  ✓ Removed existing .kilocode/'));
    }
  }
  ...
  if (fs.existsSync(COMMANDS_DIR)) {
    ...
    if (userCommands.length > 0) {
      console.log(chalk.green(`  ✓ Loaded ${userCommands.length} agent(s) from ~/.katana/commands/`));
    }
  }
  ...
  // Generate AGENTS.md (KiloCode's native memory file)
  ...
  console.log('');
  console.log(chalk.bold.green('  ✓ Katana agent initialized in .kilocode/'));
